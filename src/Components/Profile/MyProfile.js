    ...
    function conten(postData,dataProject){
        let res;
            ...
            return (res.map((x) => 
            {
                ...
                return(
            <div className='w-32 h-32 lg:w-full lg:h-64 2xl:w-full 2xl:h-92 flex border-2 overflow-hidden shadow-xl'>
    ...
    const setProfilePicture = async (e) =>{
        ...
        try {
          await fetch('http://192.168.100.13:8000/setting/settingPictureProfile',{
            ...
            .then((res) => res.json())
            .then((data) => ( fetchData() ,setMessage('Profile picture has been change')) )
        } catch (error) {
            console.log(error);
        }
    }
    ...
    <div className='h-auto bg-slate-50 pb-52 xl:w-3/4 2xl:w-1/2 mx-auto'>
      <title>My Profile</title>
         <div className='container flex flex-col justify-center items-center mx-auto '>
            ...
            {/* Search Collumnt */}
                <div className={`${search == true ? `flex flex-col` : `hidden`} justify-center px-4 items-center w-full 2xl:w-2/4 mt-40 2xl:top-0 fixed mx-auto`}>
                    ...
                    <div className=' shadow-2xl overflow-y-auto flex flex-col justify-center items-center bg-slate-500 bg-opacity-80 gap-2 w-full max-h-58 py-2'>
                        ...
                        {resultSearch.map((x) => (
                            ...
                            <div onClick={() => navigate(`/profile/${x._id}`)} className='flex gap-2 pb-4 items-center place-items-center border-b border-slate-400 hover:text-zinc-500 hover:bg-slate-100 w-2/3 2xl:w-96 py-1'>
                                   <div className='w-12 h-12 ml-16  border border-slate-50 rounded-full flex items-center justify-center overflow-hidden'>
                                        <img className=' bg-center bg-cover' src={x.setting.profilePicture} />
                                   </div>
