...
const socket = io('http://192.168.100.13:9000')
...
// Fetch Data Timeline
  const getData = async () => {
    ...
    try{
      ...
      .then(response => ( console.log(response),setImg(response)))
    }
    catch(err){
      console.log(err)
    }
    ...
    <div className='grid grid-cols-4 w-full shadow-xl md:w-2/4 2xl:w-1/3 mx-auto overflow-x-hidden bg-gray-200 h-full'>
      ...
      {/* content */}
        <div onClick={() => setNav(false)} className='col-span-3 h-fit w-80 -ml-8 md:w-3/4 '>
            ...
            <LazyLoadComponent>
              {img.map((a) => (<Content profile={a.post.userId == Auth._id ? () => navigate('/myprofile'):() => navigate(`/Profile/${a.post.userId}`)} profilePicture={a.user.setting.profilePicture} username={a.user.name} like={() => like(a.post)} key={a.post._id} img={a.post.url}/>))}
            </LazyLoadComponent>
            </div>
        </div>
